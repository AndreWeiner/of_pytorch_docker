...
ARG FOAM_PATH=/usr/lib/openfoam/openfoam2406
RUN apt-get update && apt-get install --no-install-recommends -y \
    openfoam2406-default && \
    echo ". ${FOAM_PATH}/etc/bashrc" >> /etc/bash.bashrc && \
    sed -i "s/-std=c++14/-std=c++17/g" ${FOAM_PATH}/wmake/rules/General/Gcc/c++ && \
...
RUN wget -q -O libtorch.zip https://download.pytorch.org/libtorch/cpu/libtorch-cxx11-abi-shared-with-deps-2.5.0%2Bcpu.zip && \
